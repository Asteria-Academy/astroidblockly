...
class KolosalApiService {
  ...
  Future<String> getChatCompletion(List<ChatMessage> history) async {
    ...
    debugPrint('KolosalAPI: Using API key: ${_apiKey.substring(0, 10)}...');
    debugPrint('KolosalAPI: API key length: ${_apiKey.length}');
    ...
    final List<Map<String, String>> messages = history
        .map((msg) => {'role': _roleToString(msg.role), 'content': msg.text})
        .toList();
